...
void const* gpu_computeVertexAttributeDataPointer(
  ...
  if (head->enabled == 0)
    ...
    return gl_VertexID * head->stride + head->buffer + head->offset;
}
...
void gpu_runVertexPuller(
  ...
  for (size_t i = 0; i < MAX_ATTRIBUTES; i++) {
    size_t x = gpu_computeGLVertexID(puller->indices, vertexShaderInvocation);
    if (gpu_computeVertexAttributeDataPointer(&puller->heads[i], x) == NULL)
      output->attributes[i] = 0;
    else
      output->attributes[i] = gpu_computeVertexAttributeDataPointer(&puller->heads[i], x);
  }
}
...
void gpu_runPrimitiveAssembly(
  ...
  GPUVertexPullerOutput m;
  GPUVertexShaderInput o;

   for (size_t i = 0; i < nofPrimitiveVertices; i++) {
     gpu_runVertexPuller(&m,puller,baseVertexShaderInvocation+i);
     o.gl_VertexID = gpu_computeGLVertexID(puller->indices,baseVertexShaderInvocation+i);
     o.attributes = &m;
     vertexShader(&primitive->vertices[i],&o,gpu);
     primitive->nofUsedVertices++;
  }
...
void gpu_computeScreenSpaceBarycentrics(
  ...
  (void)lines;
    ...
    float alfa = (d11 * d20 - d01 * d21) / denom;
